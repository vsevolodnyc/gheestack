...
Generated by /ghee on 2026-03-26
...
A compiled TypeScript binary (`design/dist/design`) that wraps the OpenAI Images/Responses API, callable from skill templates via `$D` (mirroring the existing `$B` browse binary pattern). Priority integration order: /ghee → /plan-design-review → /design-consultation → /design-review.
...
4. Priority: /ghee first, then /plan-design-review, /design-consultation, /design-review.
...
- Scoped 48-hour prototype: shared `visual_mockup.ts` utility, /ghee + /plan-design-review only, hero mockup + 2 variants
...
**1. /ghee** — Replace the Visual Sketch section
...
| `ghee/SKILL.md.tmpl` | Replace Visual Sketch section with `{{DESIGN_MOCKUP}}` |
...
- Running `/ghee` on a UI idea produces actual PNG mockups alongside the design doc
...
5. Running `/ghee` on a UI idea produces mockups inline
...
- You prioritized /ghee first — the upstream leverage point. If the brainstorm produces real mockups, every downstream skill (/plan-design-review, /design-review) has a visual artifact to reference instead of re-interpreting prose.
...
|--------|---------|-----|------|--------|----------|
| Office Hours | `/ghee` | Design brainstorm | 1 | DONE | 4 premises, 1 revised (Codex: opt-in->default-on) |
