...
```
If a design doc exists (from `/ghee`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
from a prior CEO review session that paused so the user could run `/ghee`. Use it
...
sure," or is clearly exploring rather than reviewing — offer `/ghee`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /ghee is designed for. Want to run /ghee right now?
> We'll pick up right where we left off."

Options: A) Yes, run /ghee now. B) No, keep going.
